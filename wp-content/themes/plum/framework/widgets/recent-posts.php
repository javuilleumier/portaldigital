...
class plum_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'plum_rp', // Base ID
			__('Plum Recent Posts with Thumbnails', 'plum'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'plum' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'plum' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'plum' );
		}
		?>
		<p>
		...
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'plum' ); ?></label>
...
add_action( 'widgets_init', 'register_plum_widget' );  
function register_plum_widget() {  
    register_widget( 'plum_Recent_Posts' );  
}  
